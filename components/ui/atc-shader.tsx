...
// Blue-tinted variant of the ATC shader
...
void main(){
  ...
  // Shift palette to deep blue
  float lum = dot(o.rgb, vec3(0.299, 0.587, 0.114));
  o.r = lum * 0.08;
  o.g = lum * 0.22 + o.b * 0.1;
  o.b = clamp(lum * 1.4 + 0.04, 0.0, 1.0);
...
export default function AtcShader({ className }: { className?: string }) {
  ...
  return (
    <div className={`relative w-full h-full ${className ?? ""}`}>
      <canvas ref={canvasRef} className="block w-full h-full" style={{ background: "#000" }} />
      <pre ref={errRef} className="absolute top-2 left-2 text-green-400 text-xs whitespace-pre-wrap" />
    </div>
  )
}
